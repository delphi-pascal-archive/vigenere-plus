...
function UpdateCRC32(InitCRC: cardinal; BufPtr: Pointer; Len: word): cardinal; assembler;
...
function UpdateCRC32(InitCRC: cardinal; BufPtr: Pointer; Len: word): cardinal; assembler;
asm
      or     edx, edx           // Data prüfen, bei 0 ->
      je     @@exit
      jecxz  @@exit             // cbData prüfen, bei 0 ->
      push   ebx
@@m1:
      xor    ebx,ebx            // ebx := Result and $000000FF
      mov    bl,al
      xor    bl, BYTE [edx]     // bl  := Data[] xor bl
      shr    eax, 8             // eax := Result shr 8
      xor    eax, dword [CRC32Table] + [EBX*4]
      inc    edx                // Inc(Buf)
      dec    ecx
      jnz    @@m1               // next byte
      pop    ebx
  ...
  try
    ...
    on E: Exception do
      ...
      if Assigned(InFile) then
        InFile.Free;
      ShowMessage(Format('Îרטבךא ''%s'' ןנט מבנאבמעךו פאיכא ''%s''',
        [FileName, E.Message]));
      end;
    end;
  Result := not crc32;
